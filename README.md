...
**중요**:
- 일반 메시지 전송: `[ID]` 형식이 필수는 아니지만 권장됩니다.
- `--wait` 패턴 사용 시: 반드시 `[]`로 감싸야 합니다 (예: `[작업#001]`)
- offset 기반으로 새 메시지를 자동으로 구분합니다.
...
```bash
# 메시지 전송 후 바로 응답 대기 (NEW!)
NoljiMa "작업 완료" --wait

# 패턴과 함께 메시지 전송 후 응답 대기 (NEW!)
NoljiMa "작업 시작" --wait "[작업#001]"
# → "[작업#001] 작업 시작" 형태로 전송되고, "[작업#001]" 패턴 응답 대기

# 타임아웃 지정 (초 단위) (NEW!)
NoljiMa "빌드 시작" --wait "[빌드#001]" --timeout 600

# 응답 대기만 (메시지 전송 없이)
NoljiMa --wait

# 특정 패턴 포함 메시지 대기 (패턴은 반드시 [] 로 감싸야 함)
NoljiMa --wait "[작업#001]"
...
# 워크플로우 예시
NoljiMa "코드 리뷰 필요" --wait "[리뷰#001]"  # 1. 전송 + 응답 대기 (한 번에!)
# 사용자가 텔레그램에서 "[리뷰#001] 확인완료" 입력
# → NoljiMa 종료 (exit 0)
```
...
**NoljiMa는 Global Mutex를 사용하여 병렬 실행을 자동으로 방지합니다.** 이미 NoljiMa가 실행 중인 상태에서 다시 실행하면 자동으로 차단됩니다.
...
# 터미널 2
NoljiMa "메시지"
# → 오류: NoljiMa가 이미 실행 중입니다.
# → 병렬 실행이 금지되어 있습니다.
```

**이유**:
- `offset.txt`는 "어디까지 읽었는지" 저장하는 단일 파일입니다
- 여러 프로세스가 동시에 접근하면 메시지 중복 처리나 누락이 발생합니다
- Global Mutex로 시스템 전체에서 단 하나의 인스턴스만 실행되도록 보장합니다
