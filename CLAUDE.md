# NoljiMa 프로젝트 규칙

## 언어
- 한국어 기본

## 문서
- 모든 문서는 `docs/` 폴더에 위치
- 명세서: `docs/NoljiMa.md`
- 릴리스 이력: `CHANGELOG.md` (루트에 단일 파일로 관리)

## 기술 스택
- .NET 8.0 Console App
- HttpClient (Telegram API)

## 스킬 자동 실행 규칙

### submit 스킬
다음 표현이 나오면 자동으로 submit 스킬을 실행:
- "작업 내용을 올리자"
- "작업 올리자"
- "작업 올려줘"
- "작업 제출"
- "커밋하고 푸시"

submit 스킬은 현재 변경사항을 커밋하고 원격 저장소에 푸시합니다.

## Git 규칙

### 세션 시작 시 검증
Claude Code 세션 시작 시 반드시 git 상태를 확인하고 최신화:

```bash
# 원격 저장소 최신 정보 가져오기
git fetch

# 현재 상태 확인
git status

# 로컬 변경사항이 없고 원격이 앞서 있으면 pull
# (변경사항이 있으면 사용자에게 확인)
```

이를 통해 항상 최신 코드 베이스에서 작업을 시작합니다.

### commit 전 검증
git에 올리기 전에 반드시 nul 파일 존재 여부를 확인하고 삭제:

```bash
# nul 파일 검색 및 삭제
find . -name "nul" -type f -delete 2>/dev/null
git status
```

Windows에서 잘못된 리다이렉션으로 생성될 수 있는 "nul" 파일을 방지합니다.

## 파일 정리 규칙

### .gitignore에서 제외할 빌드 출력
- `publish/` - dotnet publish 출력 폴더
- `NoljiMa-v*.exe` - 배포용 실행 파일
- `NoljiMa-v*.zip` - 배포용 압축 파일

### 배포 파일 명명 규칙
- 포터블 실행 파일: `NoljiMa-v{버전}-portable.zip`
- 독립 실행형: `NoljiMa-v{버전}-standalone.zip`
- 예시: `NoljiMa-v1.0.0-portable.zip`
